...
export function generatorFn() {
    ...
    program.action(() => {
        ...
        const subcommand: GeneratorSubcommand = args._.length
            ? args._
        ...
        if (subcommand === "generate" || subcommand[0] === "generate") {
            ...
            let generatorName: OpenApiGenerator =
                generateArgs.g || generateArgs["generator-name"];
            GenerateDefaultOptions.map((item: DefaultOptionType) => {
                const defaultOptionArgs: GenerateArgumentType = minimist(
                    item.key
                );
                if (
                    !Object.keys(defaultOptionArgs).some((key: string) =>
                        Object.keys(generateArgs).some(
                            (argKey: string) => argKey !== "_" && argKey === key
                        )
                    )
                ) {
                    if (
                        item?.key?.indexOf(GenerateOptionName.generatorName) >
                        -1
                    ) {
            ...
            if (generateArgs.p || generateArgs["additional-properties"]) {
                const additionalProps: keyof GenerateArgumentType = !!generateArgs.p
                    ? "p"
                    : "additional-properties";
                const formattedAdditionalProps: string = removeExtraOption(
                    args[additionalProps]
                );
                args[additionalProps] = `${formattedAdditionalProps}${
                    formattedAdditionalProps.length ? "," : ""
            ...
            if (isSEBGenerator(generatorName)) {
                const relativeGenerator: OpenApiGenerator = customGeneratorMapper.find(
                    ({ generator }: CustomGeneratorMapper) =>
                        generatorName === generator
                )?.relativeGenerator;
                if (relativeGenerator) {
                    (args as GenerateArgumentType)[
                        (args as GenerateArgumentType).g
                            ? "g"
                            : "generator-name"
                    ] = relativeGenerator;
                }
        ...
        CustomOptions.filter(({ option }: CustomOptionType) => !!option).map(
            (item: CustomOptionType) => {
                const customOption: CustomOptionsArgumentType = minimist(
                    item.option
                );
                const selectedKey: string = Object.keys(
                    customOption
                ).find((key: string) =>
...
function removeExtraOption(addtionalProperties: string): string {
    ...
    additionalArgs.map((argument: string) => {
        ...
        const relatedOption: CustomOptionType = CustomOptions.find(
            (option: CustomOptionType) =>
                !!option.mappingName && option.mappingName === keyName
        );
        ...
        (option: CustomOptionType) => {
            ...
            if (option.additionalProp) {
                const additionalProps: string =
                    (args as GenerateArgumentType).p ||
                    (args as GenerateArgumentType)["additional-properties"];
                if (
                    !additionalProps ||
                    additionalProps.indexOf(option.mappingName) === -1
                ) {
                    argumentValue = option.defaultValue;
                }
            } else {
                const extraArgs: CustomOptionsArgumentType = minimist(
                    option.option
                );
                const extraDependentArgs: CustomOptionsArgumentType = minimist(
                    option.dependedOption || []
                );
                const extraArgument: string = Object.keys(
                    args
                ).find((key: string) =>
                ...
                const extraDependentArgument: string = Object.keys(
                    args
                ).find((key: string) =>
                    Object.keys(extraDependentArgs).some(
                        (argKey: string) => argKey !== "_" && argKey === key
                    )
                );
                if (
                    option.dependedOption &&
                    !!args[extraArgument] !== !!args[extraDependentArgument]
                ) {
